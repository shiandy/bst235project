...
```{r plot-betas, fig.height = 8, cache = TRUE}
...
par(mfcol = c(1, 2))
for (i in 1:2) {
    plot(testsim_lst[[i]]$naive_preds, testsim_lst[[i]]$test_ys,
         pch = 20, col = rgb(0, 0, 0, 0.3),
...
```{r exploration-preds, fig.height = 8, cache = TRUE}
par(mfcol = c(3, 2))
for (i in 1:2) {
    plt_lims <- c(min(testsim_lst[[i]]$test_ys),
                  max(testsim_lst[[i]]$test_ys))
